...
func (s *Server) tryExtractMatplotlibPlot(projectID, command, output string) map[string]interface{} {
	...
	if strings.Contains(output, ".png") || strings.Contains(output, "saved") {
		...
		for _, plotPath := range plotPaths {
			if plotData := s.readPlotFromContainer(projectID, plotPath); plotData != "" {
...
func main() {
	...
	http.HandleFunc("/ws", func(w http.ResponseWriter, r *http.Request) {
		...
		// Use authenticated WebSocket handler
...
<body>
	...
	if server.Host == "10.0.0.1" {
		log.Printf("🔒 VPN Mode: Accepting on all interfaces; use wss://10.0.0.1:%s/ws from clients", server.Port)
		log.Printf("🏠 Local Fallback also available: wss://%s:%s/ws", server.getLocalIP(), server.Port)
	} else {
		log.Printf("🏠 Local Mode: Binding to all interfaces (0.0.0.0:%s)", server.Port)
		log.Printf("🌐 Local Access: wss://%s:%s/ws", server.getLocalIP(), server.Port)
	}

	// Start server with TLS
	log.Printf("🔐 TLS/HTTPS enabled with TLS 1.2+")
	log.Printf("🌐 Web interface: https://%s:8080", server.getLocalIP())
	log.Printf("🔑 Login endpoint: https://%s:%s/api/login", server.getLocalIP(), server.Port)
	log.Printf("🎯 Ready for secure connections on %s...", bindAddr)
...
// Enhanced staged execution helper functions
